...
if __name__ == "__main__":
    ...
    class Gaussian2DDataset(Dataset):
        def __init__(self, num_samples=2000):
            ...
            self.cov = torch.tensor([[1.0, 0.0, 0.4], [0.0, 1.0, 0.4], [0.4, 0.4, 1.0]])
            self.data = torch.distributions.MultivariateNormal(
    ...
    if hyperparameter_tune == True:
        ...
        lambda_penalty_params = (
            False  # torch.FloatTensor([study.best_params["lambda_penalty_params"]])
        )
    else:
    ...
    model.pretrain_tranformation_layer(
        dataloader,
        iterations=100,
        max_batches_per_iter=False,
    ...
    print(
        (synth_samples.T.cov() @ torch.diag(1 / torch.diag(synth_samples.T.cov())))
        .numpy()
        .round(3)
    )
