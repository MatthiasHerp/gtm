...
create_rvine_structure_matrix <- function(D) {
  M <- RVineMatrixSample(d=D, size = 1, naturalOrder = FALSE)[[1]]
  return(M)
}
...
create_copula_families_matrix <- function(D, I_percent=0.2, Independence_Tree=2, gauss_only=FALSE) {
  ...
  # - took out all copulas that only have negative correlation as this is the positive only case
...
create_copula_par_matrix <- function(D, family, par2=FALSE, mintau=0.3, maxtau=0.7) {
  ...
  for (row in 1:D) {
    for (col in 1:D) {
      if (col < row) { # no diagonal elements
        if (family[row, col] %in% c(1) ){ #Gauss which can be positive or negative, here restricted to positive
          mat[row, col] <- runif(1, min = BiCopTau2Par(1,mintau), max = BiCopTau2Par(1,maxtau))# * sample(c(1,-1),1)
          
        } else if (family[row, col] %in% c(2)){ #T which can be positive or negative, here restricted to positive
        ...
        } else if (family[row, col] %in% c(5) ){ #Frank which can be positive or negative, here restricted to positive
...
create_sim_study_data <- function(path_string,
                                  ...
                                  num_seeds = 1,
                                  grid_obs_num = 40000,
                                  folder="rvinet1_posonly_10_4000",
                                  gauss_only=FALSE){
  ...
  for (seed_value in seeds){
    ...
    rvinematrix <- create_rvine_structure_matrix(D_dimensions)
    ...
    write.table(kendall_tau_matrix,paste0(num,"_kendalltaumatrix.csv"), row.names = FALSE, col.names = FALSE, sep=",")
