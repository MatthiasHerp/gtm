...
def adjust_ploynomial_range(polynomial_range: object, span_factor: object) -> object:
    ...
    span_change = torch.tensor([-span_factor*span, span_factor*span], dtype=torch.float32, device=polynomial_range.device)
...
class ReLULeR(): #nn.Module
    def __init__(self,polynomial_range: torch.Tensor):
        #super().__init__()
        self.polynomial_range = 0.9*polynomial_range # this factor helps stabilize bounds of the decorrelation layer spline predictions
        self.l0 = nn.ReLU()
        self.l1 = nn.ReLU()
