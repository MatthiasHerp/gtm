...
def bernstein_first_derivative_batched(x, theta, n, binom):
    ...
    basis = bernstein_basis_batched(x, n - 1, binom)#binomial_coeffs(n - 1, x.device))  # (B, N, D-1)
...
def bernstein_second_derivative_batched(x, theta, n, binom):
    ...
    basis = bernstein_basis_batched(x, n - 2, binom)#binomial_coeffs(n - 2, x.device))  # (B, N, D-2)
...
def bernstein_prediction_vectorized(params_a, input_a,
                       degree, spline_range, knots=None, monotonically_increasing=False, 
                       derivativ=0, return_penalties=False, calc_method="does_not_matter",
                       span_factor=0.1, span_restriction="reluler",
                       covariate=False, params_covariate=False, covariate_effect="multiplicativ",
                       penalize_towards=0, order=3, varying_degrees=True,params_a_mask=None, binom_n=None, binom_n1=None, binom_n2=None): #device=None
    ...
    if derivativ == 0:
        prediction = bernstein_prediction_batched(input_a_clone.T, params_a.T, n=n, binom=binom_n)         # (B, N)
    elif derivativ == 1:
        prediction = bernstein_first_derivative_batched(input_a_clone.T, params_a.T, n=n, binom=binom_n1)   # (B, N)
    ...
    if return_penalties:
        ...
        second_order_ridge_pen = torch.sum(torch.diff(params_a,n=2,dim=0)**2)#torch.mean(second_derivativ**2)
        first_order_ridge_pen = torch.sum(torch.diff(params_a,n=1,dim=0)**2) #torch.mean(first_derivativ**2)
        ...
        return prediction.T.squeeze(), second_order_ridge_pen, first_order_ridge_pen, param_ridge_pen
    else:
