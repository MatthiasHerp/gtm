...
def log_likelihood(model, samples, mean_loss=False, return_lambda_matrix=True):
    # train_covariates=False, train=True, evaluate=True,

    return_dict_nf_mctm = model.forward(
        samples, return_lambda_matrix=return_lambda_matrix, linear_extrapolation_transformation_layer=True
        ...
        if adaptive_lasso_weights_matrix is False:
            pen_lambda_lasso = (
                (
                    ...
                    * 1 / precision_matrix.size(0) * (torch.square(precision_matrix).sum(0)) ** 0.5
                )
                .tril(-1)
                .mean()
            )
