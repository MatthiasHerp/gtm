...
def plot_conditional_dependence_pair(
    ...
    axes[0, 1].hexbin(
        ...
        C=conditional_correlation_matrix_resampled_samples[:, sample_indices[0], sample_indices[1]].detach(),
        gridsize=30, cmap='icefire', reduce_C_function=np.mean, norm=norm
    )

    # ----------- Bottom row: \tilde{Z} -----------
    resampled_samples_after_transformation = loaded_model.after_transformation(resampled_samples).detach()

    sns.kdeplot(
        x=resampled_samples_after_transformation[:, sample_indices[0]],
        y=resampled_samples_after_transformation[:, sample_indices[1]],
        fill=True, alpha=0.9, ax=axes[1, 0]
    )
    ...
    # ----------- Colorbar -----------
    if show_colorbar:
        fig.colorbar(hb, cax=cax, orientation='vertical', label='Conditional Correlation')
