...
def compute_precision_matrix_summary_statistics(precision_matrix_train):
    ...
    #
    precision_matrix_summary_statistics["var_row"] = (
        torch.tril_indices(data_dims, data_dims, offset=-1)[0, :].numpy().flatten()
    )
    precision_matrix_summary_statistics["var_col"] = (
        torch.tril_indices(data_dims, data_dims, offset=-1)[1, :].numpy().flatten()
    )
    #
